...
AutoTurnCmd::AutoTurnCmd(DriveSubBase *pDrive, double absoluteAngle, double speed) 
{
  m_pDrive = pDrive;
...
void AutoTurnCmd::Initialize() 
{
  ...
  // if NO IMU, then exit
  if (m_pDrive->IsImuAvailable() == false)
  {
    m_isFinished = true;
    return;
  }
...
void AutoTurnCmd::Execute() 
{
  ...
  m_isFinished = (m_pDrive->GetIMUAngle() >= m_absoluteAngle);
}
...
void AutoTurnCmd::End(bool interrupted) 
{
  if (m_pDrive != nullptr)
  {
    m_pDrive->MoveTank(0.0, 0.0); // stop motors
  }
}
